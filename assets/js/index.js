...
jQuery(document).on('click', '#back', function () {  
	...
	mousePos = jQuery('#back').offset().top + 20;  
	jQuery('.color-picker-panel').addClass('active').css({'top': mousePos, 'opacity': 1});  
...
jQuery(document).on('click', '#text', function () {  
	...
	mousePos = jQuery('#text').offset().top + 20;  
	jQuery('.color-picker-panel').addClass('active').css({'top': mousePos, 'opacity': '1'});
});
...
function getHueColor(e){
  e.preventDefault();
  var y = e.pageY - hueRect.top - mousePos;
